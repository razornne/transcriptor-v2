...
- ✅ **2026-09-25 сделано в коде:** сетка v2 (Free / Pro $12·$9·249 ₴ / Team $14·$11·229 ₴), Max и Privacy Mode убраны, своя квота диктовки (миграция 016), общий пул минут Team, `managed_payments` в Checkout за флагом, вебхук больше не путает планы, `/app?upgrade=pro|team` с лендинга ведёт в оплату. **Осталось владельцу:** миграция 016 → цены (`scripts/stripe_plans_v2.py --apply`) → секрет → деплой; заявка на Managed Payments в Dashboard → `STRIPE_MANAGED_PAYMENTS=on`; Terms/Privacy — про Link как продавца; сборка приложения 0.4.1 (квота диктовки в окне).
