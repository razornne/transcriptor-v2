...
- ✅ **2026-09-25 сделано в коде:** сетка v2 (Free / Pro $12·$9·249 ₴ / Team $14·$11·229 ₴), Max и Privacy Mode убраны, своя квота диктовки (миграция 016), общий пул минут Team, `managed_payments` в Checkout за флагом, вебхук больше не путает планы, `/app?upgrade=pro|team` с лендинга ведёт в оплату. 2026-09-26: цены v2 созданы в Stripe, бэкенд и фронт выкачены. Миграция 016 применена, Managed Payments включён и проверен, Terms/Privacy обновлены. **Осталось:** сборка приложения 0.4.1 (квота диктовки в окне); первая реальная покупка — проверить вебхук и Telegram-уведомление.
...
- ✅ 2026-09-26: Privacy Policy и лендинг описывают архив записей (R2, 90 дней), реплеи без маскировки и всех обработчиков; удаление аккаунта стирает аудио в R2.
- Владельцу: опубликовать Google OAuth (сейчас Testing — чужие не войдут через Google), своя SMTP для magic link (Resend), ящик privacy@skriptly.io, реальная тестовая покупка с возвратом.
...
### ~~Stripe — subscription.updated pro/max разграничение~~ ✅ снято 2026-09-25
Max убран — личная подписка только Pro; вебхук больше не доверяет metadata.plan. `past_due` не отбирает доступ, пока Stripe повторяет списание.
