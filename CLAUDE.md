...
### Живой транскрипт во время записи (с 2026-09-24)
...
- Шорткат диктовки настраивается (`settings.hotkey` — виртуальные коды). `save_settings` шорткат не трогает (окно держит старую копию настроек — затирало). Плашка показывает только анимацию записи, без текста; в покое — капсула внизу экрана (`show_bar`, прячется над полноэкранными окнами).
